...
check(version == "0.3.0", "VERSION is 0.3.0")

compose = (ROOT / "docker-compose.yml").read_text(encoding="utf-8")
check("build:" not in compose, "compose uses prebuilt image only")
check("ghcr.io/thetruezerotwo/remote-service:v0.3.0" in compose, "compose pins the current release image")
...
check('version = "0.3.0"' in pyproject, "pyproject version matches VERSION")
check('requires-python = ">=3.10,<3.15"' in pyproject, "pyproject declares supported Python range")
...
for required in ["README.md", "DEVELOPMENT.md", "SECURITY.md", "CHANGES.md", "LICENSE", "VERSION", ".env.example", ".gitattributes", ".editorconfig"]:
